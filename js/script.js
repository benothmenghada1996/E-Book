...
function dynamicHeader(){
    ...
    if (connectedUserId){
        if (user.role=="client"){
            innerHeader = `<li class="active"><a href="Home.html">Home</a></li>
            ...
            <li><a href="index.html" onclick ="signOut()"><i class="fa fa-sign-out fa-lg"></i> Sign-Out </a></li>` 
        }
        if (user.role=="Owner" || user.role=="admin") {
            innerHeader =  ` <a href="index.html" class="navbar-brand d-flex d-lg-none me-4">
            <h2 class="text-primary mb-0"><i class="fa fa-hashtag"></i></h2>
        </a>
        <a href="#" class="sidebar-toggler flex-shrink-0">
            <i class="fa fa-bars"></i>
        </a>
        <div class="d-none d-md-flex ms-4 mx-3">
            <input class="form-control border-0" type="search" id="searchHouseInput" placeholder="Search">
            <button class="btn btn-primary mx-2" onclick="searchHouseBy()">OK</button>
        </div>
        <div class="navbar-nav align-items-center ms-auto" style="margin-left: 30%;" >
            <div class="nav-item dropdown" >
                <a href="#" class="nav-link dropdown-toggle::after" data-bs-toggle="dropdown">
        ...
        <div style="margin-left: 3%;">
            <a href="index.html" onclick ="signOut()"><i class="fa fa-sign-out fa-lg "></i> Sign-Out</a>
        </div>`
        }   
    }
    else {
...
// Ajout des maisons d'hotes par un owner (addHouse-Dash.html)
...
location.replace("OwnerHousesTab.html");
}
}
...
else{document.getElementById("roomNbError").innerHTML = "You are able to add maximum 5 rooms to your house !";}
}
...
function displayRoomDetails(){
    ...
    var innerRoom="";
        ...
        <div class="rd-text">
            ...
            <div class="row col-lg-12 mt-3 mb-3"><h5 class="ms-2 classPrice">${room.price}</h5><h5 class="mx-1 classPrice">TND</h5><h6 class="">/Night</h6></div>
            <table>
                <tbody>
                    <tr>
...
function bookingRoom(){
    ...
    innerError (testAvailability, "availabilityError", "This room is not available for this period !");
    var guestsNumber = document.getElementById("guestsNumber").value;
    var isValidGuestsNumber = compare(guestsNumber,room.capacity);
    innerError (isValidGuestsNumber, "GuestNumberError", "Your guests Number exceeds room capacity !");
    if (!testAvailability && !isValidGuestsNumber){
            var reservation ={ 
                id : reservationId,
                idRoom :idRoom,
                idHouse : HouseId,
...
// Affichage les maisons d'hotes d'un owner et toutes les maisons pour l'admin
     ...
     var user = searchObj("users",connectedUserId);
        if (user.role == "Owner"){
            if (connectedUserId == GuestHousesTab[i].ownerId){
                ...
                <button class="btn btn-primary"  onclick="modifHouse(${GuestHousesTab[i].id})">Edit</button>
                <button class="btn btn-warning" onclick="goToObj('displayedGuestHouse',${GuestHousesTab[i].id},'OwnerRoomsTab.html')"><i class="fa fa-eye me-2"></i> Rooms</button>
                <button class="btn text-white mt-2" style="background-color:#DCA73A; border-color:#DCA73A" onclick="goToObj('displayedGuestHouse',${GuestHousesTab[i].id},'addRooms-Dash.html')">
        ...
        if (user.role == "admin"){  
            ...
            <td>${owner.lName}</td>
            <td>
            <button class="btn text-white mb-2" style="background-color:#0E2E50; border-color:#0E2E50" onclick="deleteByIndex('GuestHouses',${i})">Delete</button>
            <button class="btn btn-warning" onclick="goToObj('displayedGuestHouse',${GuestHousesTab[i].id},'admin-Rooms.html')"><i class="fa fa-eye me-2"></i>Rooms</button>
            </td>
            </tr> `
        }
    }
...
// fct qui nous amène vers la page (editGuestHouse.html) et crée un clé qui prend l'id de la maison à modifier 
function modifHouse(id){
...
function editGuestHouse(){ 
    ...
    for (let i = 0; i < GuestHousesTab.length; i++) {
        ...
        <h6 class="mb-4 text-center" style="font-weight: bolder; font-size: 20px;">Edit GuestHouse</h6>
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id ="updateName" value="${GuestHousesTab[i].Name}">
        </div>
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id ="updateAdress" value="${GuestHousesTab[i].adress}">
        </div>
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id ="updateCityName" value="${GuestHousesTab[i].city}">
        </div>
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id ="updateHouseImage" value="${GuestHousesTab[i].imageURL}">
        </div>
        <div class="form-floating">
            <input class="form-control" id ="updateDescription" value="${GuestHousesTab[i].description}"></input>
        </div>
        <button type="submit" class="btn btn-primary py-3 w-100 mb-4 mt-3" onclick="updateGuestHouse(${GuestHousesTab[i].id})">Update</button>
        </div>`
        }
    }
...
// Affichage de toute les chambres appartenant à la maisons d'hotes en question
function myRooms(){
    var roomsTab= JSON.parse(localStorage.getItem("Rooms") || "[]");
    var HouseId=localStorage.getItem("displayedGuestHouse");
    var connectedUserId=localStorage.getItem("connectedUser");
    ...
    if (HouseId == roomsTab[i].HouseId){
        ...
        if (user.role == "Owner"){
            if (house.ownerId == connectedUserId){ 
                content=content + `
                <button class="btn text-white mb-2" style="background-color:#0E2E50; border-color:#0E2E50" onclick="deleteByIndex('Rooms',${i})">Delete</button>
                <button class="btn btn-primary" onclick="modifRoom(${roomsTab[i].id})">Edit</button>
                </tr> `
           } 
        }  
        if (user.role == "admin"){
...
function modifRoom(id){
    ...
    location.replace("editRooms.html");  
}
function editRoom(){ 
    var roomsTab= JSON.parse(localStorage.getItem("Rooms"));
    ...
    for (let i = 0; i < roomsTab.length; i++) {
        ...
        content= `<div class="bg-light rounded h-100 p-4">
        <h6 class="mb-4 text-center" style="font-weight: bolder; font-size: 20px;">Edit Room</h6>
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id ="updateRoomName" value="${roomsTab[i].Name}"">
        </div>
        <div class="form-floating mb-3">
            <input type="number" class="form-control" id ="updateRoomPrice" value="${roomsTab[i].price}">
        </div>
        <div class="form-floating mb-3">
            <input type="number" class="form-control" id ="updateRoomCapacity" value="${roomsTab[i].capacity}">
        </div>
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id ="updateRoomImage" value="${roomsTab[i].imageURL}">
        </div>
        <div class="form-floating">
            <input class="form-control" id="updateRoomDescription" value="${roomsTab[i].description}" rows="4"></input>
        </div>
        <button type="submit" class="btn btn-primary py-3 w-100 mb-4 mt-3" onclick="updateRoom(${roomsTab[i].id})">Update</button>
    </div>`
        }
}
document.getElementById("roomToEdit").innerHTML= content; 
}
    ...
    for (let i = 0; i < reservationsTab.length; i++) {
        ...
        if (user.role == "client"){
            if (reservationsTab[i].clientId == connectedUserId){
                innerReserv = innerReserv+ `<tr id="reservId"> 
                ...
                <td>
                <button class="btn text-white" style="background-color:#0E2E50; border-color:#0E2E50" onclick="cancelReservById(${reservationsTab[i].id})">Cancel</button>
                <button class="btn text-white mt-1" style="background-color:#0E2E50; border-color:#0E2E50" onclick="checkstatusReserv(${reservationsTab[i].id})">Check</button>
                </td>
                </tr>`
            }
        } 
        if (user.role == "Owner"){
            if (guestHouse.ownerId == connectedUserId){
                ...
                <td>${reservationsTab[i].guestsNumber}</td>
                <td>
                <button class="btn text-white mb-2" style="background-color:#0E2E50; border-color:#0E2E50" onclick="acceptReservation(${reservationsTab[i].id})">Accept</button>
                <button class="btn text-white" style="background-color:	#262626; border-color:#262626" onclick="declineReservation(${reservationsTab[i].id})">Decline</button>
                </td>
                </tr>`
            }
        }        
...
function searchHouseBy(){ 
    ...
    document.getElementById("errorSearch").style.color ="red";}
    else {
        if (user.role == "Owner") {
        var HouseId=localStorage.getItem("displayedGuestHouse");
        var house = searchObj("GuestHouses",HouseId);
            if (connectedUserId == house.ownerId){
            localStorage.setItem("searchedHouse",searchInput);
            location.replace("Owner-SearchPage.html");} 
        }
        if(user.role == "admin"){localStorage.setItem("searchedHouse",searchInput);
        location.replace("admin-SearchPage.html");}
        if(user.role == "client"){localStorage.setItem("searchedHouse",searchInput);
...
function checkDateInterval( checkIn1, checkOut1,checkIn2, checkOut2) {
    ...
    var newCheckOut2 = new Date(checkOut2).getTime;  
            if (newCheckOut1 < newCheckIn2 || newCheckOut2 < newCheckIn1){ 
                return(false);
            }   
  
    return(true);
    // return (newCheckOut1 == newCheckIn2 || newCheckOut2 == newCheckIn1)
}
...
function deleteById(key,id) {
    var canceledReservId = JSON.parse(localStorage.getItem("canceledReserv"))
    var T = JSON.parse(localStorage.getItem(key));
    var position
    for (let i = 0; i < T.length; i++) {
      if (T[i].id === id) {
...
function cancelReservById(id){   
    const cancelReserv = document.getElementById("reservId").innerHTML="";
    localStorage.setItem("cancelledReserv",id);
    alert("Are your sure you want to cancel your reservation ?");
}
// Fonction pour accepter une réservation par l'owner
function acceptReservation(idReservation) {
    localStorage.setItem("acceptedReserv",idReservation); 
}
// Fonction pour refuser une réservation par l'owner
function declineReservation(idReservation) {
    localStorage.setItem("declinedReserv",idReservation); 
}
// Fonction pour vérifier l'acceptation ou le refus d'une réservation 
function checkstatusReserv(idReservation) {
    var reservation= searchObj("reservations",idReservation);
    var acceptedReservId=localStorage.getItem("acceptedReserv"); 
    var declinedReservId=localStorage.getItem("declinedReserv"); 
    if (reservation.id==acceptedReservId){
        // alert("Your reservation was accepted");
    const messageAcceptation = "Your reservation was accepted";
        const ligneReservation = document.getElementById("reservId");
        if (ligneReservation) {
            const messageElement = document.createElement("p");
            messageElement.textContent = messageAcceptation;
            ligneReservation.appendChild(messageElement); }
    } 
    if (reservation.id==declinedReservId){
        alert("Your reservation was declined !");
    }         
}
